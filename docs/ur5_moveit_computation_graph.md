...
> **历史 mock 模式说明**：本文中的 UR5 示例现在仅允许 mock hardware。
> 下文提到的“真实驱动”是概念上的控制链，不是使用这些 UR5 命令控制真实 UR7e 的许可。
> 真实 UR7e 必须使用 [独立操作指南](ur7e_real_hardware.md)，默认缩放为 0.05，
> 且规划与执行分离；不要将本文的 home、目标值或 0.10 缩放直接用于真实机器人。
